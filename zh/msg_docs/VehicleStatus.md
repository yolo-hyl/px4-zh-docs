...
uint64 nav_state_timestamp # 当前nav_state激活时间

uint8 nav_state_user_intention                  # 用户选择的模式(在安全模式下可能与nav_state不同)

uint8 nav_state                                 # 当前激活模式
...
uint8 NAVIGATION_STATE_AUTO_PRECLAND = 20       # 精密着陆(带着陆目标)
uint8 NAVIGATION_STATE_ORBIT = 21               # 轨道飞行
...
# 当前机体移动方式。机体可能有不同方式(例如VTOL从旋翼到固定翼的转换)
uint8 vehicle_type
...
uint8 FAILSAFE_DEFER_STATE_WOULD_FAILSAFE = 2 # 延迟安全模式，但会触发安全模式

bool failsafe # 如果系统处于安全模式则为true(例如：RTL、悬停、终止等)
bool failsafe_and_user_took_over # 如果系统处于安全模式但用户已接管控制则为true
uint8 failsafe_defer_state # 为FAILSAFE_DEFER_STATE_*之一

# 链接丢失
bool gcs_connection_lost              # 与地面站的数据链丢失
uint8 gcs_connection_lost_counter     # 统计独特地面站连接丢失事件
bool high_latency_data_link_lost # 如果高延迟数据链(例如RockBlock Iridium 9603遥测模块)丢失则为true

# VTOL标志
bool is_vtol             # 如果系统具备VTOL能力则为true
bool is_vtol_tailsitter  # 如果系统在MC到FW转换时进行90°俯冲旋转则为true
bool in_transition_mode  # 如果VTOL正在进行转换则为true
bool in_transition_to_fw # 如果VTOL正在进行从MC到FW的转换则为true
...
bool safety_off # 如果安全模式关闭则为true

bool power_input_valid                            # 如果输入电力有效则设置
bool usb_connected                                # 如果USB连接则设置
